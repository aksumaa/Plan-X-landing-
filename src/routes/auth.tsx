...
import { AppLangProvider, useT } from "@/lib/planx/app-i18n";
...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Log in to the PlanX workspace to design floor plans, 3D concepts and cost estimates.",
      },
      { property: "og:title", content: "Sign in — PlanX Architecture AI" },
      {
        property: "og:description",
        content: "Access your PlanX projects, AI floor plans, 3D studio and construction estimates.",
      },
    ...
    <div className="grid min-h-screen bg-background lg:grid-cols-2">
      <div className="relative hidden items-end bg-ink p-12 lg:flex">
        ...
        <div className="relative">
          ...
          <h2 className="display mt-8 max-w-sm text-4xl text-primary-foreground">
            {t.auth.title}
          </h2>
      ...
      <div className="flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-sm">
          <Link to="/" className="mb-10 inline-flex items-center gap-2 text-[0.75rem] text-muted-foreground hover:text-foreground">
            <ArrowLeft className="size-3.5" /> {t.nav.backToSite}
          </Link>

          <h1 className="display text-3xl">{mode === "login" ? t.auth.login : t.auth.signup}</h1>
...
async function routeAfterAuth(navigate: ReturnType<typeof useNavigate>) {
  ...
  if (data && data.length > 0) navigate({ to: "/dashboard", replace: true });
